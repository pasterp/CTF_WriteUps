...
def sbox_ope(binary):
    print("sbox_ope(" + str(binary) + ")")
    for i in range(len(binary)):
        index = int(binary[i],2)
        binary[i] = sbox[index]

def phase1(binary):
    print("phase1(" + str(binary) + ")")
    m = int(len(binary) / 32)
    tmp = []
...
def phase2(binary):
    print("phase2(" + str(binary) + ")")
    for i in range(1,len(binary)):
        for j in range(i):
            binary[i] = xor(binary[i], binary[j])

def bits2hex(binary):
    print("bits2hex(" + str(binary) + ")")
    hex_str = ""
    for bit in binary:
...
def h(m):
    ...
    padding(binary)
    print("padded binary:" + str(binary))
    if len(binary) > 32:
...
def hex2bits(hashed):
    array = [hashed[i:i+2] for i in range(0, len(hashed), 2)]
    binary = []
    for bit in array:
        binary.append("{:08b}".format(int(bit, 16)))
    return binary

def sbox_reverse(hashed):
    binary = []
    for i in range(len(hashed)):
        binary.append("{:08b}".format(sbox.index(hashed[i])))
    return binary

def unphase2(binary):
    for i in range(1, len(binary)):
        binary[i] = xor(binary[i], binary[i-1])

def bin2text(binary):
    text = ""
    for i in range(len(binary)):
        text += chr(int(binary[i], 2))
    return text

hashed = "18f2048f7d4de5caabd2d0a3d23f4015af8033d46736a2e2d747b777a4d4d205"
#hashed = "9282aa47fb6f9d7c636363636363636363636363636363636363636363636363" # test
hex = hex2bits(hashed)
print(hex)
bin = sbox_reverse(hex)
print(bin)
unphase2(bin)
unphase2(bin)
unphase2(bin)
